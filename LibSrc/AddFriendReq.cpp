...
AddFriendReq::AddFriendReq(const std::string &friendName, const std::string &curr_acc)
    : _reqHead(type::addFriend, muduo::Timestamp::now().toFormattedString()), _friendName(friendName), _curr_acc(curr_acc)
{
}
...
Response AddFriendReq::handler() const
{
        ...
        if (res.empty())
        {
                resp.set(type::addFriend, true, "send friend application success");
        }
        else
        {
                resp.set(type::addFriend, false, res);
        }
        return resp;
}